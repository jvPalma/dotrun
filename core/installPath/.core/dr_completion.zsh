...
# Zsh completion for dr
_dr() {
  local -a commands flags scripts folders
  local BIN_DIR="${DR_CONFIG:-$HOME/.config/dotrun}/bin"
  ...
  commands=(
    ...
    'collections:Manage script collections from GitHub'
  )
  ...
  local CONFIG_DIR="${DR_CONFIG:-$HOME/.config/dotrun}/config"
  ...
  case $CURRENT in
    ...
    3)
      ...
      case "${words[2]}" in
        ...
        collections)
          # Collections subcommands
          local collections_commands=(
            'add:Add GitHub repository URL to collections'
            'list:List configured collection URLs'
            'remove:Remove collection URL by number'
          )
          compadd -a collections_commands
          ;;
        -l|-L)
    ...
    4)
      ...
      case "${words[2]}" in
        ...
        collections)
          case "${words[3]}" in
            add)
              # Allow typing GitHub URL - no specific completion
              ;;
            list|remove)
    ...
    *)
      ...
      if [[ ! "${words[2]}" =~ ^(add|edit|help|move|rename|mv|aliases|config|collections|-l|-L|-h|--help)$ ]]; then
